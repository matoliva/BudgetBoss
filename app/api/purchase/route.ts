...
export async function POST(request: Request) {
  try {
    const {description, amount, purchaseDate, userId, planId, frequency} =
      await request.json()
    ...
    const reminders = generateRemindersForNext3Months(
      purchaseDate,
      userId,
      purchase.id,
      frequency,
    ...
    return NextResponse.json(
      {purchase, paymentAmount, reminders},
      {status: 201},
    )
  } catch (error) {
    console.error('Error creating purchase and reminders:', error)
    return NextResponse.json(
      {error: 'Failed to create purchase and reminders'},
      {status: 500},
    )
  }
}
...
function generateRemindersForNext3Months(
  purchaseDate: string,
  userId: number,
  purchaseId: number,
  frequency: 'week' | 'fortnight' | 'month',
  ...
  for (let i = 0; i < 3; i++) {
    // Only generate for the next 3 months
